...
def email_nancy(text_file):
    ...
    #you = me
    msg['Subject'] = 'SEPHORA CANADA OUT OF STOCK ALERT'
    msg['From'] = me
    msg['To'] = you
...
oos_flag=False
email_filename = 'oos_emails/sephora_can_oos_email_' + str(datetime.now().date()) + '.txt'
...
if os.path.isfile(email_filename):
    ...
    for p in prod_urls:
        print p
        try:
            resp=requests.get(urlroot+p+'?country_switch=ca&lang=en',verify=False)
            ...
            if d['path']=='RegularProductTop':
                ...
                if 'regularChildSkus' in prod.keys():
                    ...
                    for sku in prod[field]:
                        ...
                        if sku['isOutOfStock']:
                            oos_flag=True
                            text_file.write("\nOOS: {0} {1}\n".format(urlroot+sku['targetUrl']+'?country_switch=ca&lang=en', sku['skuName']))
                    ...
                    if sku['isOutOfStock']:
                        oos_flag=True
                        text_file.write("\nOOS: {0}\n".format(urlroot+sku['targetUrl']+'?country_switch=ca&lang=en'))
                    if sku['isOnlyFewLeft']:
                        oos_flag=True
                        text_file.write("\nFEW_LEFT: {0}\n".format(urlroot+sku['targetUrl']+'?country_switch=ca&lang=en'))
